...
if ($mode == "delete" && is_numeric($_POST['delete-id'])) { 
	...
	if ($stat) {
		...
		cm_error(gettext("Error in 'cm_delete_page' function."));
		exit;
	}	
}
...
if ($mode == "edit") { 
	if (is_numeric($_POST['id']))
	{
		...
		if ($stat) {
			...
			cm_error(gettext("Error in 'cm_edit_page' function."));
			exit;
		}
	} elseif (!empty($_POST)) {
		cm_error(gettext("Did not have a page to load."));
		exit;
	}
}
...
{ 
	...
	if ($stat) {
		...
		cm_error(gettext("Error in 'cm_add_page' function."));
		exit;
	}
}
...
if ($mode == "edit" && is_numeric($id))
{
	...
	if ($result->RecordCount() != 1) {
		cm_error(gettext("Page does not exist, or you do not have permission to edit it."));
		exit;
	}
...
<h2><a href="<?php echo $pmodule; ?>.php?"><?php echo gettext("Page Manager"); ?></a></h2>
<form action="<?php echo "$module.php?action=$mode"; ?>" method="post">
  <fieldset class="<?php echo "$module-form"; ?>">
  <legend><?php echo gettext("Page Editor"); ?></legend>
  <div class="sidebar">
    <p>
      <label for="slug"><?php echo gettext("Page Slug"); ?></label>
      <br />
    ...
    <p> <strong>HTACCESS <?php echo gettext("Entry"); ?></strong> (copy) <br />
  ...
  <p>
    <label for="short_title"><?php echo gettext("Short Title"); ?></label>
    <br />
  ...
  <p>
    ...
    <p>
    <label for="side_text"><?php echo gettext("Side Text"); ?></label>
    <br />
    <span class="article-edit-tags"><script type="text/javascript">edToolbar();</script></span>
...
if ($mode == "add") {
?>
    <input type="submit" value="<?php echo gettext("Submit Page"); ?>" name="submit" id="submit" class="button" />
    <?php
}
if ($mode == "edit") {
?>
    <input type="submit" value="<?php echo gettext("Update Page"); ?>" name="update" id="update" class="button" />
...
if ($mode == "edit") { ?>
<h2><a name="preview"></a><?php echo gettext("Preview Page"); ?></h2>
<fieldset class="<?php echo "$module-preview" ?>">
<legend><?php echo gettext("Page Preview"); ?></legend>
...
<h2><?php echo gettext("Delete Page"); ?> <a href="javascript:toggleLayer('deleteRecord');" name="delete">&raquo;&raquo;</a></h2>
<div id="deleteRecord">
  <form action="<?php echo "$module.php?action=delete"; ?>" method="post">
    ...
    <p><?php echo gettext("Are you sure you want to delete this page?"); ?></p>
